...
class MatchMetricsModule:
    ...
    def highest_scorer(self):
        highScore = get_top_scoring_matchup(self.db, self.week)
        team = get_roster(self.db, highScore.roster_id)
        player = get_user(self.db, team.owner_id)
        return player.team_name(), highScore.points

    def lowest_scorer(self):
        lowScore = get_lowest_scoring_matchup(self.db, self.week)
        team = get_roster(self.db, lowScore.roster_id)
        player = get_user(self.db, team.owner_id)
        return player.team_name(), lowScore.points
